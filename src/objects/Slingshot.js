...
export default class Slingshot {
	...
	onKeyDown(event) {
		if (this.bird instanceof WhiteBird || this.bird instanceof YellowBird)
			this.bird.onKeyDown(event);
	}

	update(dt) {
		this.bird.update(dt);

		if (this.bird.isEggCreated) {

			this.eggs.push(this.bird.egg);
			this.bird.isEggCreated = false;
		}
	...
	shouldUnloadEgg(egg) {
		return (egg.didStop() || egg.didGoOffScreen());
	}
